...
def up_containers(
    ...
    **kwargs,
    ...
    """
    if containers:
        common.print_success(f"Bring up {', '.join(containers)} containers")
    else:
        common.print_success("Bring up all containers")
    up_cmd = (
        f"docker-compose up "
        f"{'-d ' if detach else ''}"
        f"{' '.join(containers)}"
...
def stop_containers(context, containers):
    """Stop containers."""
    common.print_success(f"Stopping {' '.join(containers)} containers ")
...
def clear(context):
    ...
    """
    common.print_success("Clearing docker-compose")
